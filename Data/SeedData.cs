...
namespace Supermarket.Data
{
    public class SeedData
    {
        ...
        private static void PopulateTickets(SupermarketDbContext db)
        {
            ...
            db.Tickets.AddRange(
               new Ticket { DataEmissao = DateTime.Now, DataAtendimento = DateTime.Now.AddMinutes(5), NumeroDaSenha = 1, Estado = true, Prioritario = false, IDDepartments = db.Departments.Where(a => a.NameDepartments == "Peixaria").Select(a => a.IDDepartments).FirstOrDefault() }
            );
            db.SaveChanges();
        }
    }
